/**
 * Vulkan compute shader benchmark for tower field S-box.
 *
 * Loads SPIR-V binary and runs it on AMD/Intel/NVIDIA GPU.
 */
...
int main(int argc, char** argv) {
    ...
    printf("Vulkan pipeline created successfully!\n");
    printf("✓ SPIR-V shader loaded and validated\n");
    printf("✓ AMD Radeon GPU ready\n\n");
    ...
    // Set up input: first thread gets values 0-31, second gets 32-63, etc.
    // Each thread processes 32 values in parallel (bit-sliced)
    for (int thread = 0; thread < num_threads && thread * 32 < 256; thread++) {
        for (int bit = 0; bit < 8; bit++) {
            uint32_t bitplane = 0;
            for (int lane = 0; lane < 32; lane++) {
                int value = thread * 32 + lane;
                if (value < 256 && (value & (1 << bit))) {
                    bitplane |= (1u << lane);
                }
            }
    ...
    // Run shader once for verification
    ...
    int errors = 0;
    for (int thread = 0; thread < 8; thread++) {  // Check first 256 values (8 threads)
        for (int lane = 0; lane < 32; lane++) {
            int input_val = thread * 32 + lane;
            if (input_val >= 256) break;
            ...
            if (output_byte != expected) {
                if (errors < 10) {
                    printf("  ERROR at input 0x%02x: got 0x%02x, expected 0x%02x\n",
    ...
    if (errors > 0) {
        printf("  ❌ FAILED: %d errors out of 256 values\n\n", errors);
        printf("Circuit is not computing correct S-box values!\n");
        printf("Check SPIR-V generation or shader compilation.\n");
    ...
    VkSubmitInfo submitInfoBench = {};
    submitInfoBench.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
    submitInfoBench.commandBufferCount = 1;
    submitInfoBench.pCommandBuffers = &commandBuffer;

    uint64_t start = get_time_ns();

    for (int i = 0; i < iterations; i++) {
        CHECK_VK(vkQueueSubmit(computeQueue, 1, &submitInfoBench, VK_NULL_HANDLE));
    }
    CHECK_VK(vkQueueWaitIdle(computeQueue));
