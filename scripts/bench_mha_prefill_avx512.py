#!/usr/bin/env python3
"""Multi-head GQA prefill (bf16), the complete model at the real architecture.

The full decoder-only LLM forward pass (prefill / prompt processing) with
grouped-query multi-head attention instead of the single-head attention in
bench_llm_prefill_avx512.py:

    x      = embed(ids)                  (S x D, tied VxD bf16 table)
    for l in L: MHA decoder layer        (LN, fused QKV, per-head QK^T/
                                         softmax/PV with H_q query heads and
                                         H_kv shared kv heads, concat,
                                         out-proj, residual, LN, MLP)
    x      = layernorm(x)
    logits = x . E^T                     (LM head, tied embeddings)

The layer is the reviewed layer_mha from scripts/bench_mha_avx512.py
...
Usage:
  python scripts/bench_mha_prefill_avx512.py [--seq 512] [--d 128]
...
def gen_c(
    seq: int, d: int, hm: int, layers: int, heads: int, kv_heads: int, vocab: int
) -> str:
...
static void layer_mha(
    ...
    for (int g = 0; g < HKV; g++) {
        ...
        for (int h = g; h < HQ; h += HKV) {
            ...
            for (int r = 0; r < S; r++)
                for (int j = 0; j < DH; j++) f_o[r*D + h*DH + j] = o_h[r*DH + j];
        }
    }
    to_bf16(f_o, b_o, S*D);
...
int main(void) {
    ...
    static uint16_t E[@VD@], Wqkv[@LD3@], Wo[@LD2@], W1[@LH@], W2[@LH@];
    ...
    static float f_up[@SH@], f_g[@S2@], fq[@S2@], fk[@S2@], fv[@S2@], o_h[@SDH@];
    static float logits[@SV@];  /* S x V LM-head output */
    static uint16_t b_q[@S2@], b_k[@SDH@], b_v[@SDH@], b_qh[@SDH@], b_o[@S2@], b_p[@SS@], b_up[@SH@], b_x[@S2@];
    ...
    for (int rep = 0; rep < 5; rep++) {
        ...
        for (int l = 0; l < L; l++)
            layer_mha(xbuf, wbuf, b_q, b_k, b_v, b_qh, b_o, b_p, b_up,
                      ...
                      Bp1_all + l*@SU@, Bp2_all + l*@SDW@, Bpk, Bpv);
        for (int i = 0; i < S*D; i++) wbuf[i] = xbuf[i];
        layernorm(wbuf, S, D);
        to_bf16(wbuf, b_x, S*D);
        gemm(b_x, Bp_head, logits, S, V, D);  /* LM head */
        uint64_t t1 = __rdtsc();
        uint64_t d = t1 - t0;
        if (d < best) best = d;
    }
...
def run_c(
    ...
    with tempfile.TemporaryDirectory() as td:
        td = Path(td)
        cfile = td / "mha_prefill.c"
        cfile.write_text(gen_c(seq, d, hm, layers, heads, kv_heads, vocab))
        asm_file = td / "gemm.S"
        ...
        if r.returncode != 0:
            ...
            sys.exit(1)
        out = subprocess.run(
            [str(td / "mha_prefill")], capture_output=True, text=True, cwd=td
        )
        if out.returncode != 0:
            print(out.stderr)
            sys.exit(1)
...
def numpy_reference(
    ...
    return {"t_total": t_total, "state": x.astype(np.float32)}
...
def main() -> int:
    ...
    if HQ * DH != args.d:
        raise SystemExit("heads must divide D")
    if HQ % HKV != 0:
        raise SystemExit("kv-heads must divide heads (GQA)")
    tsc = 3.7e9

    print(
        f"bf16 {L}-layer GQA prefill: S={S} D={D} H_q={HQ} H_kv={HKV} "
        f"D_head={DH} V={V}"
    )
    res = run_c(S, D, args.hm, L, HQ, HKV, V)
    ref = numpy_reference(S, D, args.hm, L, HQ, HKV, V)
    ...
    print(
        f"final hidden state max abs err vs bf16-input fp32 numpy: {abs_err:.5f} "
        f"(state scale {np.abs(ref['state']).max():.3f})"
    )

    total_ms = res["cyc_total"] / tsc * 1e3
    print(
        f"ours:   {L} layers + head in {total_ms:6.2f} ms "
        f"({total_ms/L:5.2f} ms/layer-equiv)"
    )
    print(f"        {S/(total_ms/1e3)/1e3:7.1f} k tokens/s")
    print(
        f"numpy:  {ref['t_total']*1e3:6.2f} ms ({ref['t_total']/(total_ms/1e3):5.1f}x slower)"
    )
    print(
        f"(single-head prefill at the same shapes: ~8.6 ms for S=512 L=6, "
        f"see bench/bench_llm_prefill_avx512_results.md)"
    )
    return 0
