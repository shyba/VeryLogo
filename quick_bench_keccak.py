...
def main():
    ...
    print(f"Running quick benchmark on {out_dir}...")
    print(f"Message: {msg!r}")

    got = simulate_keccak_512_once_steps(lib, layout, msg, idle_chunk=64)
    if got != exp:
    ...
    simulate_keccak_512_once_steps(lib, layout, msg, idle_chunk=64)
    ...
    print(f"  Throughput: {dps:.1f} digests/second")
    print(f"\nBaseline (from previous run): 762.1 digests/s")
    if dps > 762.1:
        improvement = (dps / 762.1 - 1) * 100
        print(f"  Improvement: +{improvement:.1f}%")
    else:
        regression = (1 - dps / 762.1) * 100
