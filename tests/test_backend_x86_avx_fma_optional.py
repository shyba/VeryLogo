...
@unittest.skipUnless(
    ...
    and _has_cpu_flag("avx")
    and _has_cpu_flag("fma"),
    "requires cc, x86_64, avx, and fma",
)
class TestBackendX86AvxFmaOptional(unittest.TestCase):
    def test_emit_and_run_avx_fma_matches_interpreter(self) -> None:
        t32 = SimdType(lane_width=32, lanes=8)
        t64 = SimdType(lane_width=64, lanes=4)
        ir = TickIR(
            name="fma",
            inputs={
                "x32": t32,
        ...
        self.assertIn("_mm256_fmadd_pd", c)
        ...
        x32 = pack(
            [_f32_bits(v) for v in [1.0, -2.0, 3.5, -4.25, 0.25, 10.0, -0.5, 2.0]], 32
        )
        y32 = pack(
            [_f32_bits(v) for v in [0.5, 1.25, -1.0, 2.0, -4.0, 0.125, 3.0, -0.75]], 32
        )
        z32 = pack(
            [_f32_bits(v) for v in [2.0, -1.0, 0.0, 1.0, 0.5, -2.0, 4.0, 0.25]], 32
        )
        x64 = pack([_f64_bits(v) for v in [1.0, -2.0, 0.5, 10.0]], 64)
        y64 = pack([_f64_bits(v) for v in [3.0, 0.25, -4.0, -0.125]], 64)
        z64 = pack([_f64_bits(v) for v in [0.0, 1.0, -1.0, 2.0]], 64)
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            for name in input_order:
                x = env[name]
                words.extend([(x >> (64 * i)) & ((1 << 64) - 1) for i in range(4)])
